...
function CategoriesPagination({
  ...
  return (
    <nav className="mt-12 flex items-center justify-center gap-1.5" aria-label={t("categories.pagination.label")}>
      <Button
        variant="outline"
        size="icon"
        disabled={currentPage <= 1}
        onClick={() => onPageChange(currentPage - 1)}
        aria-label={t("categories.pagination.previous")}
      ...
      <Button
        ...
        onClick={() => onPageChange(currentPage + 1)}
        aria-label={t("categories.pagination.next")}
        className="rounded-none border-2"
      >
